...
import static org.elo.connection.EloContextParameter.*;
...
public class EloConnectionDetails {
    ...
    public EloConnectionDetails(EloConnectionManager eloConnectionManager) {
        this(eloConnectionManager, null);
    }
    ...
    public EloConnectionDetails(EloConnectionManager eloConnectionManager, Properties callContext) {
        ...
        if (callContext == null) {
            ...
            detailMap.put(ELO_CONNECTION_FACTORY_CONNECTION_APP_USER, eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_CONNECTION_APP_USER, ""));
            detailMap.put(ELO_CONNECTION_FACTORY_CONNECTION_APP_PASSWORD, eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_CONNECTION_APP_PASSWORD, ""));
        } else {
            //prepare detailMap with context details relevant for elo connections
            for (String key : EloContextParameter.getEloContextParameters()) {
                Object value = callContext.get(key);
                if (key != null) {
                    detailMap.put(key, value);
                }
            }
            detailMap.put(AUTHENTICATION_TYPE, EloAuthenticationType.fromValue((String) callContext.get(AUTHENTICATION_TYPE)).name());
        }
    ...
    public IXConnFactory createIXConnFactory() throws de.elo.utils.net.RemoteException {
        ...
        String url = eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_PROPERTY_URL, "");
        int nbOfCnns = Integer.parseInt(eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_PROPERTY_NB_OF_CNNS, "0"));
        int timeoutSeconds = Integer.parseInt(eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_PROPERTY_TIMEOUT_SECONDS, "0"));
        Properties connectionProperties = IXConnFactory.createConnProps(url, nbOfCnns, timeoutSeconds);

        // set session options
        String appName = eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_SESSION_APP_NAME, "");
        String appVersion = eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_SESSION_APP_VERSION, "");
    ...
    private ClientInfo createClientInfo() {
        ...
        String defaultCountry = eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_CONNECTION_CLIENT_COUNTRY, Locale.getDefault().getCountry());
        String defaultLanguage = eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_CONNECTION_CLIENT_LANGUAGE, Locale.getDefault().getLanguage());
        String defaultTimeZone = eloConnectionManager.getServiceProperties().getProperty(ELO_CONNECTION_FACTORY_CONNECTION_CLIENT_TIMEZONE, TimeZone.getDefault().getID());
    ...
    public IXConnFactory getIXConnFactory() throws de.elo.utils.net.RemoteException {
        //return this.eloConnectionManager.getIxConnFactory();
        ...
        try {
            if (((String) detailMap.get(AUTHENTICATION_TYPE)).equals(AUTHENTICATION_TYPE_SERVER_APPLICATION)) {
                ...
                ixConnection = ixConnFactory.create(
                        ...
                        computerName,
                        "");
            } else if (((String) detailMap.get(AUTHENTICATION_TYPE)).equals(EloAuthenticationType.BASIC.name())) {
                ...
                ixConnection = ixConnFactory.create(
                    ...
                    ((String) detailMap.get(AUTHENTICATION_TYPE)).equals(EloAuthenticationType.BASIC_AS.name())) {
                getLogger().debug("User: " + (String) this.detailMap.get(USER));
                getLogger().debug("Password: " + (String) this.detailMap.get(PASSWORD));
            ...
            } else if (((String) detailMap.get(AUTHENTICATION_TYPE)).equals(EloAuthenticationType.TICKET.name())) {
                ixConnection = ixConnFactory.createFromTicket(this.clientInfo);
            } else if (((String) detailMap.get(AUTHENTICATION_TYPE)).equals(EloAuthenticationType.KERBEROS.name())) {
